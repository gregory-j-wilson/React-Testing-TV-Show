...
import {findByTestId, render, screen, waitFor, fireEvent, wait} from '@testing-library/react'
...
const showData = {
  ...
  test('renders show data when the API call finishes', async () => {
    ...
    // await for the api call, and query for the show data to be rendered
    const showTitle = await screen.findByTestId(/show-name/i)
    expect(showTitle).toBeInTheDocument()

})
    // // query for and click on the "select a season" button

test('Select option from drop-down and display episodes', async () => {
  ...
  const season1 = screen.getByText(/chapter one/i)
